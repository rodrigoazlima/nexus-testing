...
test.describe.serial('Image tags: orc1.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/orc1] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('orc1.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop orc1.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('orc1.jpg');
