...
const EXPECTED_TAGS = ['body', 'dragon', 'white'];

test.describe.serial('Image tags: dragon-white.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/dragon-white] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('dragon-white.jpg gets expected name and draft state', async () => {
    const { randomName } = await test.step('drop dragon-white.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('dragon-white.jpg');
  ...
  for (const tag of EXPECTED_TAGS) {
    test(`dragon-white.jpg tags include "${tag}"`, () => {
  ...
  test('dragon-white.jpg gets classified into the bestiary and shows on the dashboard', async ({ page }) => {
    const { data: enrichedData } = await test.step(
      'wait for the classification agent to place dragon-white.jpg in the bestiary',
      () =>
        pollNoteUntil(
          notePath,
    ...
    await test.step('assert dragon-white.jpg shows up on the dashboard bestiary page', async () => {
