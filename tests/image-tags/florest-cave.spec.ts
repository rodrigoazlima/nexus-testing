...
test.describe.serial('Image tags: florest-cave.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/florest-cave] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('florest-cave.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop florest-cave.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('florest-cave.jpg');
