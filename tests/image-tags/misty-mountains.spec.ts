...
const EXPECTED_TAGS = ['scene', 'mountains'];

test.describe.serial('Image tags: misty-mountains.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/misty-mountains] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('misty-mountains.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop misty-mountains.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('misty-mountains.jpg');
    ...
    await test.step('validate tags', () => {
      assertTagsInclude(data.tags, EXPECTED_TAGS, 'misty-mountains.jpg');
    });
  });
});
