...
test.describe.serial('Image tags: vingador.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/vingador] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('vingador.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop vingador.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('vingador.jpg');
    ...
    await test.step('validate tags', () => {
      assertTagsInclude(data.tags, EXPECTED_TAGS, 'vingador.jpg');
    });
  });
});
