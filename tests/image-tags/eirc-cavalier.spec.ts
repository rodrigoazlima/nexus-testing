...
const EXPECTED_TAGS = ['portrait', 'cavalier'];

test.describe.serial('Image tags: eirc-cavalier.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/eirc-cavalier] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('eirc-cavalier.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop eirc-cavalier.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('eirc-cavalier.jpg');
    ...
    await test.step('validate tags', () => {
      assertTagsInclude(data.tags, EXPECTED_TAGS, 'eirc-cavalier.jpg');
    });
  });
});
