...
test.describe.serial('Image tags: heman-barbarian2.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/heman-barbarian2] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('heman-barbarian2.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop heman-barbarian2.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('heman-barbarian2.jpg');
    ...
    await test.step('validate tags', () => {
      assertTagsInclude(data.tags, EXPECTED_TAGS, 'heman-barbarian2.jpg');
    });
  });
});
