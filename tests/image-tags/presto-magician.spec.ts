...
test.describe.serial('Image tags: presto-magician.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/presto-magician] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('presto-magician.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop presto-magician.jpg under a random name', async () => {
