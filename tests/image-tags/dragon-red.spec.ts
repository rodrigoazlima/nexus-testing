...
const EXPECTED_TAGS = ['body', 'dragon', 'red'];

test.describe.serial('Image tags: dragon-red.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/dragon-red] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('dragon-red.jpg gets expected name and draft state', async () => {
    const { randomName } = await test.step('drop dragon-red.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('dragon-red.jpg');
  ...
  for (const tag of EXPECTED_TAGS) {
    test(`dragon-red.jpg tags include "${tag}"`, () => {
  ...
  test('dragon-red.jpg gets classified into the bestiary and shows on the dashboard', async ({ page }) => {
    const { data: enrichedData } = await test.step(
      'wait for the classification agent to place dragon-red.jpg in the bestiary',
      () =>
        pollNoteUntil(
          notePath,
    ...
    await test.step('assert dragon-red.jpg shows up on the dashboard bestiary page', async () => {
