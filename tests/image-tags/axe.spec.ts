...
test.describe.serial('Image tags: axe.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/axe] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('axe.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop axe.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('axe.jpg');
