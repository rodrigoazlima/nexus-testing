...
test.describe.serial('Image tags: bobby-barbarian.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/bobby-barbarian] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('bobby-barbarian.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop bobby-barbarian.jpg under a random name', async () => {
