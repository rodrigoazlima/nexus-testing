...
test.describe.serial('Image tags: orc2.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/orc2] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('orc2.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop orc2.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('orc2.jpg');
