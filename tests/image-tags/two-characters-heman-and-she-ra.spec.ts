...
const EXPECTED_TAGS = ['scene', 'barbarian'];

test.describe.serial('Image tags: two-characters-heman-and-she-ra.jpg -> vision draft', () => {
  ...
  test.afterEach(async ({}, testInfo) => {
    if (testInfo.status !== testInfo.expectedStatus) {
      ...
      console.log(`[image-tags/two-characters-heman-and-she-ra] FAILED — files copied for inspection to ${dir}`);
    }
  });
  ...
  test('two-characters-heman-and-she-ra.jpg gets expected tags, name, and draft state', async () => {
    const { randomName } = await test.step('drop two-characters-heman-and-she-ra.jpg under a random name', async () => {
      const dropped = await copyFixtureWithRandomName('two-characters-heman-and-she-ra.jpg');
    ...
    await test.step('validate tags', () => {
      assertTagsInclude(data.tags, EXPECTED_TAGS, 'two-characters-heman-and-she-ra.jpg');
    });
  });
});
